...
- `configure` no longer links `-lprocstat` into both binaries on any system that happens to have it. The check is now guarded on FreeBSD, where `src/process.c` actually uses it, and the result is passed through `PROCSTAT_LIBS` instead of the global `LIBS`.
