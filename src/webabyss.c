 ...
 */
static SearchItem *
wallpaperabyss_json_obj_to_searchitem (json_object *j_obj)
{
    json_object *j_val;

    SearchItem *si_item = searchitem_new ();

    searchitem_set_service_name (si_item, ww_name (WEB_SERV_WALLABYSS));
        ...
        if (json_object_object_get_ex (j_obj, "wallpapers", &j_arr) &&
            ...
            for (i = 0; i < ui_cnt; ++i) {
                if ((j_val = json_object_array_get_idx (j_arr, i)) != NULL) {
                    si_item = wallpaperabyss_json_obj_to_searchitem (j_val);
